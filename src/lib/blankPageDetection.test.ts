...
describe('detectBlankPage', () => {
  ...
  // ── NEW: Printed worksheet metadata tests ──

  it('flags a page with only printed worksheet metadata (Level/Form/Page)', () => {
    const result = detectBlankPage(
  ...
  it('flags a page with Diagnostic Worksheet header and no student work', () => {
    const result = detectBlankPage(
      'Diagnostic Worksheet - Level C | Form C | Generated by NYCLogic Ai'
    );
    expect(result.isBlank).toBe(true);
  });

  it('flags a page with QR code JSON artifact', () => {
    const result = detectBlankPage(
      'Name:\n{"v":1,"s":"abc123","q":"q1","p":2,"t":3}\nPage 2'
    );
    expect(result.isBlank).toBe(true);
  });

  it('flags a page with only printed instructions and no student work', () => {
    const result = detectBlankPage(
