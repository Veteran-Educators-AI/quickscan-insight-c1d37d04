...
// ═══════════════════════════════════════════════════════════════════════════════
// BLANK PAGE DETECTION — strips printed boilerplate, checks meaningful content
// ═══════════════════════════════════════════════════════════════════════════════
const BOILERPLATE_PATTERNS = [
  ...
  /^work\s*[:.]?\s*$/gim,
  /^\s*[-–—_]{3,}\s*$/gm,
  // Worksheet metadata (Level/Form/Page, Diagnostic, Generated by, etc.)
  /level\s*[a-z]\s*[|,]?/gim,
  /form\s*[a-z]\s*[|,]?/gim,
  ...
  /practice/gim,
  /work\s*area/gim,
  // Common printed question fragments (not student work)
  /^(solve|find|calculate|determine|simplify|evaluate|explain|describe|compare|identify|before\s+adding|adding\s+them)\b.*$/gim,
...
function isBlankPage(rawText: string | null | undefined, threshold = 20): boolean {
  ...
  if (normalizedLength < threshold) return true;

  // Secondary check: strip ALL non-alphanumeric and check if fewer than 15 remain.
  // This catches pages where boilerplate patterns didn't strip everything but the
  // remaining text is just short fragments. Threshold is kept low (15) to avoid
  // false positives on short but real math work like "x=5" or "3x+5=20".
  const alphanumericOnly = text.replace(/[^a-zA-Z0-9]/g, "");
  if (alphanumericOnly.length < 15) return true;
    ...
    schema: {
      ...
      properties: {
        ...
        student_work_present: {
          type: "boolean",
          description:
            "Is there actual student HANDWRITING visible on the page? Printed questions, worksheet titles, headers, page numbers, QR codes, machine-printed instructions, and pre-printed labels do NOT count. Only handwritten responses by a student count. If the page has ONLY printed/typed content with blank answer areas, this MUST be false.",
        },
        detected_subject: {
          type: "string",
...
STEP 1 — EXTRACT: Read ALL student handwriting from the entire page including margins. Record exact text in "ocr_text". IMPORTANT: Only extract HANDWRITTEN text by the student — do NOT include printed questions, headers, or instructions.
STEP 2 — DETECT: Is there student handwriting? Printed questions, worksheet titles, headers ("Name:", "Date:", "Level C", "Form C"), page numbers, QR codes, and machine-printed instructions do NOT count as student work. If the answer areas are EMPTY with no handwriting, set student_work_present = false and set ALL grading booleans to false.
    ...
    if (isBlankPage(textForBlankCheck)) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          analysis: {
            ocrText: textForBlankCheck,
            studentWorkPresent: false,
            isAcademicAssignment: true,
            detectedSubject: "",
            ...
            feedback: "No student work detected on this page.",
            strengthsAnalysis: [],
